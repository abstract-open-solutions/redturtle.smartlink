        ...
        if ilink:
            ...
            if smartlink_config:
                if smartlink_config.relativelink:
                    obj = self.getField('internalLink').get(self)
                    remote = '/'.join(obj.getPhysicalPath())
                    mask = smartlink_config.relativelink_mask
                    if mask and remote.startswith(mask):
                        remote = remote[len(mask):]
